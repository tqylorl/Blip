...
void parseText() {
    // Move to the next token, which should be the text content after the 'text' keyword
    read_next_token();

    // Check if the next token is a string or word
    if (next_token_type == NAME || next_token_type == SYMBOL) {
        printf("%s\n", token);
    } else if (next_token_type == NUMBER) {
        printf("%d\n", token_number_value);
    } else {
        fprintf(stderr, "Error: Expected text after 'text' keyword.\n");
    }

    // Move to the next token for future parsing
    read_next_token();
}
